...
public class DMS {
	...
	public static String[][] Sequencer(int rowsize , int colsize , List<Node> ArrayNode){
		...
		for (int i = 0 ; i < ArrayNodeSort.size();i++){
					...
					indexpointer[i] = 	Integer.parseInt(nametemp.substring(size-1, size)) ;
			...
			while (!stopper ){	
				...
				if (row<rowsize){
					...
					valuetemp = nametemp.substring(size-1,size);
